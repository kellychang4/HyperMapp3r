...
def parsefn():
    parser = argparse.ArgumentParser(description='Generates volumetric summary of WMH segmentations',
    ...
    required.add_argument('-o', '--out_csv', type=str, metavar='',
                          help='output stats ex: wmh_vols_summary.csv', default='wmh_volumes.csv')
...
def main(args):
    ...
    for i in range(0, len(subjs_dirs)):
        ...
        if glob.glob(os.path.join(input_dir, subjs_dirs[i], '*%s' % mask_name)):
            ...
            for j in range(0, len(wmh_label)):
                volume[i, j] = np.shape(np.nonzero(mask_data == wmh_label[j]))[1] * voxel_volume
        else:
    ...
    df = df[(df.T != 0).any()]
    print('saving WMH volumetric csv')
